...
| `Zmq::monitor_event_mask(names => \@names)` | hashref | `{ mask, hex, events:[{name, flag}] }` — build the subscribe bitmask to pass to `zmq_socket_monitor` by OR-ing each event's `ZMQ_EVENT_*` flag; `all` is the catch-all `0xFFFF` (`ZMQ_EVENT_ALL`); the setup-side companion to `parse_monitor_event` |
